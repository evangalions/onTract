...
L.Util = {
    ...
    requestAnimFrame: function() {
        ...
        window.oRequestAnimationFrame || window.msRequestAnimationFrame || a;
...
L.Class.extend = function(a) {
    var b = function() { ! L.Class._prototyping && this.initialize && this.initialize.apply(this, arguments)
    };
    ...
    if (a.options && c.options) a.options = L.Util.extend({},
    c.options, a.options);
    L.Util.extend(c, a);
...
L.Bounds = L.Class.extend({
    ...
    contains: function(a) {
        var b;
        if (a instanceof L.Bounds) b = a.min,
        a = a.max;
...
L.LineUtil = {
    ...
    simplifyDP: function(a, b) {
        for (var c = 0, d = 0, e = b * b, f = 1, g = a.length, h; f < g - 1; f++) h = this._sqPointToSegmentDist(a[f], a[0], a[g - 1]),
        h > c && (d = f, c = h);
    ...
    reducePoints: function(a, b) {
        for (var c =
        [a[0]], d = b * b, e = 1, f = 0, g = a.length; e < g; e++) this._sqDist(a[e], a[f]) < d || (c.push(a[e]), f = e);
        f < g - 1 && c.push(a[g - 1]);
        return c
    },
    ...
    _getEdgeIntersection: function(a, b, c, d) {
        ...
        f = d.min,
        d = d.max;
        if (c & 8) return new L.Point(a.x + e * (d.y - a.y) /
        b, d.y);
...
L.PolyUtil.clipPolygon = function(a, b) {
    ...
    for (g = 0; g < 4; g++) {
        ...
        for (f = j - 1; e < j; f = e++) if (h = a[e], f = a[f], h._code & k) {
            if (! (f._code & k)) f = l._getEdgeIntersection(f, h, k, b),
            f._code = l._getBitCode(f, b),
            c.push(f)
        } else {
            if (f._code & k) f = l._getEdgeIntersection(f, h, k, b),
            f._code = l._getBitCode(f, b),
...
L.DomEvent = {
    addListener: function(a, b, c, d) {
        ...
        else if ("addEventListener" in a) if (b == "mousewheel") a.addEventListener("DOMMouseScroll", e, !1),
        a.addEventListener(b, e, !1);
            ...
            a.addEventListener(b == "mouseenter" ? "mouseover": "mouseout",
            e, !1)
    ...
    removeListener: function(a, b, c) {
        ...
        d = "_leaflet_" + b + c;
        handler = a[d];
        L.Browser.mobileWebkit && b == "dblclick" && this.removeDoubleTapListener ? this.removeDoubleTapListener(a, c) : "removeEventListener" in a ? b == "mousewheel" ? (a.removeEventListener("DOMMouseScroll", handler, !1), a.removeEventListener(b, handler, !1)) : b == "mouseenter" || b == "mouseleave" ? a.removeEventListener(b == "mouseenter" ? "mouseover":
        "mouseout", handler, !1) : a.removeEventListener(b, handler, !1) : "detachEvent" in a && a.detachEvent("on" + b, handler);
        a[d] = null
    },
    ...
    disableClickPropagation: function(a) {
        ...
        "mousedown", L.DomEvent.stopPropagation);
...
L.Util.extend(L.DomEvent, {
    ...
    removeDoubleTapListener: function(a, b) {
        ...
        a.removeEventListener(a, a["_leaflet_touchend" + b],
        !1)
    }
});
L.DomUtil = {
    ...
    disableTextSelection: function() {
        ...
        if (!this._onselectstart) this._onselectstart = document.onselectstart,
    ...
    setPosition: function(a, b) {
        a._leaflet_pos = b;
        L.Browser.webkit ?
        a.style[L.DomUtil.TRANSFORM] = L.DomUtil.getTranslateString(b) : (a.style.left = b.x + "px", a.style.top = b.y + "px")
    },
...
L.Draggable = L.Class.extend({
    ...
    enable: function() {
        if (!this._enabled) L.DomEvent.addListener(this._dragStartTarget, L.Draggable.START, this._onDown, this),
        this._enabled = !0
    },
    disable: function() {
        if (this._enabled) L.DomEvent.removeListener(this._dragStartTarget,
        L.Draggable.START, this._onDown),
        this._enabled = !1
    },
    _onDown: function(a) {
        if (! (a.shiftKey || a.which != 1 && a.button != 1 && !a.touches) && !(a.touches && a.touches.length > 1)) {
            ...
            L.DomEvent.addListener(document,
            L.Draggable.MOVE, this._onMove, this);
...
L.Transition = L.Transition.extend({
    ...
    run: function(a) {
        ...
        for (b in a) a.hasOwnProperty(b) && (b = d[b] ? d[b] : b, b = b.replace(/([A-Z])/g,
        function(a) {
        ...
        this.fire("start");
        L.Transition.NATIVE ?
        this._timer = setInterval(this._onFakeStep, this.options.fakeStepInterval) : this._onTransitionEnd()
    },
    _onFakeStep: function() {
        this.fire("step")
    },
    _onTransitionEnd: function() {
        if (this._inProgress) this._inProgress = !1,
...
L.Transition = L.Transition.NATIVE ? L.Transition: L.Transition.extend({
    ...
    _runFrame: function(a) {
        ...
        c,
        d;
        for (c in this._props) this._props.hasOwnProperty(c) && (d = this._props[c], c in b ? (d = d.to.subtract(d.from).multiplyBy(a).add(d.from), b[c](this._el, d)) : this._el.style[c] = (d.to - d.from) *
        a + d.from + d.unit);
        this.fire("step")
    },
    _complete: function() {
...
L.LatLngBounds = L.Class.extend({
    ...
    contains: function(a) {
        ...
        c = this._northEast,
        d;
        a instanceof L.LatLngBounds ? (d = a.getSouthWest(),
        a = a.getNorthEast()) : d = a;
...
L.Projection.Mercator = {
    ...
    unproject: function(a, b) {
        ...
        g = Math.PI / 2 - 2 * Math.atan(d), h = 15, j = 0.1; Math.abs(j) > 1.0E-7 && --h > 0;) j = f * Math.sin(g),
        j = Math.PI / 2 - 2 * Math.atan(d * Math.pow((1 - j) / (1 + j), 0.5 * f)) - g,
        g += j;
...
L.CRS.EPSG900913 = L.Util.extend({},
L.CRS.EPSG3857, {
    code: "EPSG:900913"
});
L.CRS.EPSG4326 = L.Util.extend({},
L.CRS, {
    code: "EPSG:4326",
    projection: L.Projection.LonLat,
    transformation: new L.Transformation(1 / 360, 0.5, -1 / 360, 0.5)
});
L.CRS.EPSG3395 = L.Util.extend({},
L.CRS, {
    code: "EPSG:3395",
...
L.FeatureGroup = L.LayerGroup.extend({
    ...
    _initEvents: function(a) {
        for (var b = 0, c = this._events.length; b < c; b++) a.on(this._events[b],
...
L.TileLayer = L.Class.extend({
    ...
    onAdd: function(a) {
        ...
        else this._limitedUpdate = L.Util.limitExecByInterval(this._update, 100, this),
    ...
    _initContainer: function() {
        ...
        if (!this._container || a.empty) this._container = L.DomUtil.create("div", "leaflet-layer", a),
    ...
    _addTilesFromCenterOut: function(a) {
        for (var b = [], c = a.getCenter(), d = a.min.y; d <= a.max.y; d++) for (var e = a.min.x; e <= a.max.x; e++) e + ":" + d in this._tiles ||
    ...
    _removeOtherTiles: function(a) {
        ...
        c,
        d;
        for (d in this._tiles) if (this._tiles.hasOwnProperty(d) && (b = d.split(":"), c = parseInt(b[0], 10), b = parseInt(b[1], 10), c < a.min.x || c > a.max.x || b < a.min.y || b > a.max.y)) this._tiles[d].src = "",
        this._tiles[d].parentNode == this._container && this._container.removeChild(this._tiles[d]),
    ...
    _addTile: function(a) {
        ...
        d = a.x + ":" + a.y,
        e = 1 << c;
        if (!this.options.noWrap) a.x = (a.x % e + e) % e;
        if (! (a.y < 0 || a.y >= e)) {
    ...
    getTileUrl: function(a, b) {
        return this._url.replace("{s}", this.options.subdomains[(a.x + a.y) % this.options.subdomains.length]).replace("{z}",
        b).replace("{x}", a.x).replace("{y}", a.y)
    },
...
L.Popup = L.Class.extend({
    ...
    onAdd: function(a) {
        ...
        if (this._map.options.closePopupOnClick) this._map.on("preclick", this._close,
        this);
        this._update();
    ...
    _initLayout: function() {
        this._container =
        L.DomUtil.create("div", "leaflet-popup");
        ...
        this._tipContainer = L.DomUtil.create("div", "leaflet-popup-tip-container",
        this._container);
    ...
    _adjustPan: function() {
        if (this.options.autoPan) {
            ...
            if (b.y + a > e.y) c.y = b.y + a - e.y + d.y; (c.x || c.y) && this._map.panBy(c)
        }
    },
...
L.Marker = L.Class.extend({
    ...
    _initInteraction: function() {
        ...
        if (L.Handler.MarkerDrag) this.dragging =
        new L.Handler.MarkerDrag(this),
        this.options.draggable && this.dragging.enable()
    },
    _onMouseClick: function(a) {
        L.DomEvent.stopPropagation(a); (!this.dragging || !this.dragging.moved()) && this.fire(a.type)
    },
...
L.Path = L.Class.extend({
    ...
    _initRoot: function() {
        if (!this._map._pathRoot) this._map._pathRoot = this._createElement("svg"),
...
L.Path = L.Path.SVG || !L.Path.VML ? L.Path: L.Path.extend({
    ...
    _initRoot: function() {
        if (!this._map._pathRoot) this._map._pathRoot = document.createElement("div"),
        this._map._pathRoot.className =
    ...
    _initStyle: function() {
        this.options.stroke ?
        (this._stroke = this._createElement("stroke"), this._stroke.endcap = "round", this._container.appendChild(this._stroke)) : this._container.stroked = !1;
    ...
    _updateStyle: function() {
        if (this.options.stroke) this._stroke.weight = this.options.weight + "px",
        this._stroke.color = this.options.color,
        this._stroke.opacity = this.options.opacity;
        if (this.options.fill) this._fill.color =
...
L.Path.include({
    bindPopup: function(a, b) {
        ...
        this._popup.setContent(a);
        if (!this._openPopupAdded) this.on("click", this._openPopup, this),
...
L.Polyline = L.Path.extend({
    ...
    _getPathPartStr: function(a) {
        for (var b = L.Path.VML, c = 0, d = a.length, e = "", f; c < d; c++) f = a[c],
        b && f._round(),
        e += (c ? "L": "M") + f.x + " " + f.y;
        return e
    },
    _clipPoints: function() {
        ...
        if (this.options.noClip) this._parts = [a];
            ...
            h = L.LineUtil;
            for (d = c = 0; c < b - 1; c++) if (e = h.clipSegment(a[c], a[c + 1], g, c)) if (f[d] = f[d] || [], f[d].push(e[0]), e[1] != a[c + 1] || c == b - 2) f[d].push(e[1]),
            d++
        }
    },
...
L.Polygon = L.Polyline.extend({
    ...
    initialize: function(a, b) {
        ...
        if (a[0] instanceof Array) this._latlngs = a[0],
    ...
    function a(a) {
        return L.FeatureGroup.extend({
            initialize: function(c, d) {
                this._layers = {};
                for (var e = 0, f = c.length; e < f; e++) this.addLayer(new a(c[e], d))
            },
            setStyle: function(a) {
...
L.Circle = L.Path.extend({
    ...
    getPathString: function() {
        ...
        b = this._radius;
        return L.Path.SVG ?
        "M" + a.x + "," + (a.y - b) + "A" + b + "," + b + ",0,1,1," + (a.x - 0.1) + "," + (a.y - b) + " z": (a._round(), b = Math.round(b), "AL " + a.x + "," + a.y + " " + b + "," + b + " 0,23592600")
    }
});
...
L.GeoJSON = L.LayerGroup.extend({
    ...
    addGeoJSON: function(a) {
        ...
        else b = a.type == "Feature" ? a.geometry: a,
        c = L.GeoJSON.geometryToLayer(b, this.options.pointToLayer),
...
L.Util.extend(L.GeoJSON, {
    geometryToLayer: function(a, b) {
        ...
        switch (a.type) {
        case "Point":
            return d = this.coordsToLatLng(c),
            b ? b(d) : new L.Marker(d);
        case "MultiPoint":
            e = 0;
            for (f = c.length; e < f; e++) d = this.coordsToLatLng(c[e]),
            d = b ? b(d) : new L.Marker(d),
            g.push(d);
            return new L.FeatureGroup(g);
        case "LineString":
            return c = this.coordsToLatLngs(c),
            new L.Polyline(c);
        case "Polygon":
            return c = this.coordsToLatLngs(c, 1),
            new L.Polygon(c);
        case "MultiLineString":
            return c = this.coordsToLatLngs(c, 1),
            new L.MultiPolyline(c);
        case "MultiPolygon":
            return c = this.coordsToLatLngs(c, 2),
            new L.MultiPolygon(c);
        case "GeometryCollection":
            e = 0;
            for (f = a.geometries.length; e < f; e++) d = this.geometryToLayer(a.geometries[e]),
            g.push(d);
            return new L.FeatureGroup(g);
        default:
    ...
    coordsToLatLngs: function(a, b, c) {
        ...
        f,
        g = a.length;
        for (f = 0; f < g; f++) d = b ? this.coordsToLatLngs(a[f], b - 1, c) : this.coordsToLatLng(a[f],
        c),
        e.push(d);
        return e
    }
...
L.Handler.MapDrag = L.Handler.extend({
    enable: function() {
        if (!this._enabled) {
            if (!this._draggable) this._draggable = new L.Draggable(this._map._mapPane, this._map._container),
    ...
    disable: function() {
        if (this._enabled) this._draggable.disable(),
        this._enabled = !1
    },
    moved: function() {
...
L.Handler.TouchZoom = L.Handler.extend({
    enable: function() {
        if (L.Browser.mobileWebkit && !this._enabled) L.DomEvent.addListener(this._map._container, "touchstart", this._onTouchStart, this),
        this._enabled = !0
    },
    disable: function() {
        if (this._enabled) L.DomEvent.removeListener(this._map._container, "touchstart", this._onTouchStart, this),
        this._enabled = !1
    },
    ...
    _onTouchMove: function(a) {
        if (a.touches && a.touches.length == 2) {
            if (!this._moved) this._map._mapPane.className += " leaflet-zoom-anim",
...
L.Handler.ScrollWheelZoom = L.Handler.extend({
    enable: function() {
        if (!this._enabled) L.DomEvent.addListener(this._map._container, "mousewheel", this._onWheelScroll, this),
        this._delta = 0,
        this._enabled = !0
    },
    disable: function() {
        if (this._enabled) L.DomEvent.removeListener(this._map._container, "mousewheel", this._onWheelScroll),
        this._enabled = !1
    },
    _onWheelScroll: function(a) {
        ...
        this._timer = setTimeout(L.Util.bind(this._performZoom,
        this), 50);
        L.DomEvent.preventDefault(a)
    },
...
L.Handler.DoubleClickZoom = L.Handler.extend({
    enable: function() {
        if (!this._enabled) this._map.on("dblclick", this._onDoubleClick, this._map),
        this._enabled = !0
    },
    disable: function() {
        if (this._enabled) this._map.off("dblclick", this._onDoubleClick, this._map),
        this._enabled = !1
    },
...
L.Handler.ShiftDragZoom = L.Handler.extend({
    ...
    enable: function() {
        if (!this._enabled) L.DomEvent.addListener(this._container, "mousedown", this._onMouseDown, this),
        this._enabled = !0
    },
    disable: function() {
        if (this._enabled) L.DomEvent.removeListener(this._container, "mousedown", this._onMouseDown),
        this._enabled = !1
    },
    ...
    _onMouseUp: function(a) {
        ...
        this._map.fitBounds(new L.LatLngBounds(this._map.layerPointToLatLng(this._startLayerPoint),
...
L.Handler.MarkerDrag = L.Handler.extend({
    ...
    enable: function() {
        if (!this._enabled) {
            if (!this._draggable) this._draggable = new L.Draggable(this._marker._icon, this._marker._icon),
            this._draggable.on("dragstart", this._onDragStart, this),
            this._draggable.on("drag", this._onDrag, this),
            this._draggable.on("dragend", this._onDragEnd, this);
            this._draggable.enable();
...
L.Map = L.Class.extend({
    ...
    setZoom: function(a) {
        return this.setView(this.getCenter(),
        a)
    },
    zoomIn: function() {
    ...
    getBoundsZoom: function(a) {
        ...
        f = this.project(e, c),
        g = this.project(a, c),
        f = new L.Point(f.x - g.x, g.y - f.y);
        while (f.x <= b.x && f.y <= b.y && c <= d);
        return c - 1
    },
    getSize: function() {
        if (!this._size ||
        this._sizeChanged) this._size = new L.Point(this._container.clientWidth, this._container.clientHeight),
        this._sizeChanged = !1;
        return this._size
    },
    ...
    _resetView: function(a, b, c) {
        ...
        d && this.fire("zoomend");
        this.fire("moveend");
        if (!this._loaded) this._loaded = !0,
        this.fire("load")
    },
    ...
    _initControls: function() {
        ...
        if (this.options.attributionControl) this.attributionControl = new L.Control.Attribution,
    ...
    _initEvents: function() {
        L.DomEvent.addListener(this._container, "click", this._onMouseClick,
        this);
    ...
    _onMouseClick: function(a) {
        if (!this.dragging || !this.dragging.moved()) this.fire("pre" + a.type),
    ...
    _onTileLayerLoad: function() {
        ...
        this._tileBg) clearTimeout(this._clearTileBgTimer),
...
L.Map.include({
    ...
    _handleGeolocationError: function(a) {
        var a = a.code,
        b = a == 1 ? "permission denied":
        a == 2 ? "position unavailable": "timeout";
        if (this._setViewOnLocate) this.fitWorld(),
        this._setViewOnLocate = !1;
    ...
    _handleGeolocationResponse: function(a) {
        ...
        if (this._setViewOnLocate) b = Math.min(this.getBoundsZoom(f), this._maxLocateZoom),
        this.setView(f.getCenter(), b),
        this._setViewOnLocate = !1;
...
L.Map.include(!L.Transition || !L.Transition.implemented() ? {}: {
    ...
    panBy: function(a) {
        ...
        this._panTransition.on("step", this._onPanTransitionStep,
        this),
    ...
    _panByIfClose: function(a) {
        if (this._offsetIsWithinView(a)) return this.panBy(a),
        !0;
        return ! 1
    },
...
L.Map.include(!L.DomUtil.TRANSITION ? {}: {
    ...
    _prepareTileBg: function() {
        if (!this._tileBg) this._tileBg = this._createPane("leaflet-tile-pane", this._mapPane),
    ...
    _stopLoadingBgTiles: function() {
        for (var a = [].slice.call(this._tileBg.getElementsByTagName("img")), b = 0, c = a.length; b < c; b++) if (!a[b].complete) a[b].src = "",
...
L.Map.include({
    ...
    _initControlPos: function() {
        ...
        b = L.DomUtil.create("div", "leaflet-control-container",
        this._container);
